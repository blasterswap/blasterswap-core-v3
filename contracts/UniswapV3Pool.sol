...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    constructor() {
        ...
        (_governor, factory, token0, token1, fee, _tickSpacing) = IUniswapV3PoolDeployer(msg.sender).parameters();
    ...
    function _updatePosition(
        ...
        if (token0 == address(USDB) || token1 == address(USDB)) {
            ...
            if (token0 == address(USDB)) {
                feeGrowthInside0X128 = feeGrowthInside0X128 + amountClaimedUSDB;
            }

            if (token1 == address(USDB)) {
                feeGrowthInside1X128 = feeGrowthInside1X128 + amountClaimedUSDB;
            }
        }
        ...
        if (token0 == address(WETH) || token1 == address(WETH)) {
            ...
            if (token0 == address(WETH)) {
                feeGrowthInside0X128 = feeGrowthInside0X128 + amountClaimedWETH;
            }

            if (token1 == address(WETH)) {
                feeGrowthInside1X128 = feeGrowthInside1X128 + amountClaimedWETH;
            }
        }
